...
urls <- c(
  "https://lulctimeseries.blob.core.windows.net/lulctimeseriesv003/lc2022/35J_20220101-20230101.tif"
)
for(url in urls){
  download.file(
    url = url,
...
land_cover_lesotho <- terra::subst(
  land_cover_vrt,
  from = from,
  to = to,
  names = cols
...
terra::plotRGB(land_cover_lesotho)
...
land_cover_lesotho_resampled <- terra::resample(
  x = land_cover_lesotho,
  y = terra::rast(elev),
  method = "near"
) |>
  terra::project(crs_lambert)

terra::plotRGB(land_cover_lesotho_resampled)

img_file <- "land_cover_lesotho.png"

terra::writeRaster(
  land_cover_lesotho_resampled,
  img_file,
  overwrite = T,
  NAflag = 255
)
...
filename <- "3d_land_cover_lesotho-dark.png"
...
legend_name <- "land_cover_lesotho_legend.png"
png(legend_name)
par(family = "mono")
...
magick::image_write(
  p, "3d_lesotho_land_cover_final.png"
)
